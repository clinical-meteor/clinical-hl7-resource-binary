...
JsonRoutes.add("get", "/fhir/Binary", function (req, res, next) { process.env.DEBUG && console.log('GET /fhir/Binary', req.query);
  ...
  if (accessToken || process.env.NOAUTH) {
    ...
    process.env.DEBUG && console.log('Binarys.find(id)', Binarys.find(databaseQuery).fetch()); // because we're using BaseModel and a _transform() function
    // Binarys returns an object instead of a pure JSON document // it stores a shadow reference of the original doc, which we're removing here
    var binaryData = Binarys.find(databaseQuery).fetch();
    binaryData.forEach(function(patient){
